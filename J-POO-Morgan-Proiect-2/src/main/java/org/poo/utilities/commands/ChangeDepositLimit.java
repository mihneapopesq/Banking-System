...
public class ChangeDepositLimit extends CommandBase{
     ...
     */
    public ChangeDepositLimit(final Builder builder) {
    ...
    public void execute() {
        for(BusinessAccount account : businessAccounts) {
            if(account.getOwnerEmail().equals(commandInput.getEmail())) {
                account.setDepositLimit(commandInput.getAmount());
                return;
            }
        }
        ObjectNode errorNode = objectMapper.createObjectNode();
        errorNode.put("description", "You must be owner in order to change deposit limit.");
